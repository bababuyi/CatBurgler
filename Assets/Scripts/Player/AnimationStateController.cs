...
public class AnimationStateController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        float reference = player.IsSprinting ? runReferenceSpeed : walkReferenceSpeed;
        float targetSpeed = speed > 0.1f
            ? Mathf.Clamp(speed / reference, 0.5f, 3f)
            : 1f;
        ...
        anim.SetFloat(VertHash, vert);

        float state = player.IsSprinting ? 1f : 0f;
        anim.SetFloat(StateHash, state);
    }
}